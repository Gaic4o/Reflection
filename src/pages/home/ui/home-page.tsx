...
const steps = createFunnelSteps<Partial<HomeFunnelContext>>()
  ...
  .extends("Three")
  .extends("Result", {
    requiredKeys: ["retroType", "retroAnswer1", "retroAnswer2", "retroAnswer3"],
  })
  .build();
  ...
  return (
    <article className="flex m-auto flex-col justify-center bg-[#263D2F] w-[600px]">
      <memoir.Render
        Start={({ history }) => (
          <StartClass
            onNext={(teacher) => history.push("First", { teacherNm: teacher })}
          />
        )}
        First={({ history }) => (
          <FirstPeriod
            onNext={(inputs) => history.push("Two", { ...inputs })}
          />
        )}
        Two={({ history }) => (
          <SecondPeriod onNext={() => history.push("Three")} />
        )}
        Three={({ history, context }) => (
          <ThreePeriod
            homeContext={context}
            onComplete={(props) => history.push("Result", { ...props })}
          />
        )}
