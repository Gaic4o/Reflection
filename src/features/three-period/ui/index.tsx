...
type ThreePeriodFunnelContext = Pick<HomeFunnelContext, 'retroType' | 'retroAnswer1' | 'retroAnswer2' | 'retroAnswer3'>;
...
const ThreePeriod = ({ homeContext, onComplete }: ThreePeriodProps) => {
  ...
  const [retroType, setRetroType] = useState<ThreePeriodFormProps['type'] | null>(null);
  ...
  return (
    <Container className='mx-[19px]'>
      <formFunnel.Render
          ...
          <>
            ...
            <Box as='div'>
              <Flex
                ...
                className={`flex flex-row border-2 p-[26px] rounded-lg cursor-pointer transition-all duration-300 hover:border-white/40
                  ${retroType === 'KPT' ? 'border-[#93C5FD] bg-white/10' : 'border-white/20'}`}
              >
                <Image
                ...
                />
                <Box>
                  <p className='text-[#93C5FD] text-[20px] font-semibold mb-[7px]'>KPT 회고</p>
            ...
            <Box as='div' className='mt-[26px]'>
              <Flex
                ...
                className={`flex flex-row border-2 p-[26px] rounded-lg cursor-pointer transition-all duration-300 hover:border-white/40
                  ${retroType === 'YWT' ? 'border-[#86EFAC] bg-white/10' : 'border-white/20'}`}
              >
                <Image
                ...
                />
                <Box>
                  <p className='text-[#86EFAC] text-[20px] font-semibold mb-[7px]'>YWT 회고</p>
            ...
            <div className='flex items-end flex-col mt-[67px] mr-[24px]'>
              <button
                ...
                className={`h-[50px] w-[218px] text-[#fff] text-[30px] font-medium rounded-t-[40px]
                  ${retroType ? 'bg-[#5A3625] cursor-pointer' : 'bg-[#5A3625]/50 cursor-not-allowed'}`}
              >
                NEXT
              </button>
        ...
        ANSWER={({ context }) => (
          <ThreePeriodForm type={context.retroType} homeContext={homeContext} onComplete={onComplete} />
        )}
      />
    </Container>
  );
...
const ThreePeriodForm = ({ type, onComplete, homeContext }: ThreePeriodFormProps & ThreePeriodProps) => {
  const [answers, setAnswers] = useState<
    Pick<ThreePeriodFunnelContext, 'retroAnswer1' | 'retroAnswer2' | 'retroAnswer3'>
  >({
    retroAnswer1: '',
    retroAnswer2: '',
    retroAnswer3: '',
  ...
  const handleSubmit = useCallback(() => {
    if (isAllAnswered()) {
      ...
      console.log('최종 제출 데이터:', {
        선생님: homeContext.teacherNm,
        이름: homeContext.userNm,
        팀: homeContext.team,
        경험: homeContext.experience,
        회고타입: type,
  ...
  return (
    <Container className='mx-[19px]'>
      <h1 className='text-[64px] text-center text-[#fff] font-semibold mt-[77px] mb-[37px]'>{type} 회고</h1>
      {steps.map((step, index) => (
        <article key={index}>
          <h2 className='text-[32px] text-center font-semibold mb-[43px] mt-[85px]' style={{ color: step.color }}>
            STEP{index + 1}, {step.title}
          </h2>

          <Box>
            <label className='text-[24px] font-semibold' style={{ color: step.color }}>
              직군,연차별로 AI가 생성하는 질문
            </label>
            <TextareaAutosize
              maxRows={10}
      ...
      <div className='flex items-end flex-col mt-[67px] mr-[24px]'>
        <button
          ...
          className={`h-[50px] w-[218px] text-[#fff] text-[30px] font-medium rounded-t-[40px]
            ${isAllAnswered() ? 'bg-[#5A3625] cursor-pointer' : 'bg-[#5A3625]/50 cursor-not-allowed'}`}
        >
          SUBMIT
        </button>
        <div className='h-[11px] w-[218px] bg-[#0F1048] rounded-b-[3px]'></div>
